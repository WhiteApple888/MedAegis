...
class PickPopFinderPage:
    ...
    def create_styled_table(self, columns, custom_widths=None):
        """
        :param columns: List of column names, e.g. ["Item Code", "Description", "Qty", "Price"]
        :param custom_widths: Dict mapping column name or index to width, e.g. {"Item Code": 90, "Qty": 60}
        """
        ...
        # Set default mode to Stretch (unspecified columns will absorb remaining space)
    ...
    # Build ratios using each column's header name or index.
        ratios = {}
        for index, name in enumerate(columns):
            ratio = custom_widths.get(name, custom_widths.get(index))
            ...
            # Required so setColumnWidth controls each section.
            header.setSectionResizeMode(index, QHeaderView.Interactive)

        resize_state = {"updating": False}

        def apply_column_ratios():
            if not ratios or resize_state["updating"]:
                return
            ...
            resize_state["updating"] = True

            for column_index, ratio in ratios.items():
                column_width = int(available_width * ratio / total_ratio)
                table.setColumnWidth(column_index, column_width)

            resize_state["updating"] = False

        # Reapply the ratios whenever the table/header geometry changes.
        header.geometriesChanged.connect(apply_column_ratios)

        # Apply once after Qt has put the table into its layout.
