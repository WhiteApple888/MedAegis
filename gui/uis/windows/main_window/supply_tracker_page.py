...
class SupplyTrackerPage:
    ...
    def setup_ui(self):
        ...
        self.qty_label_2 = self.create_label(text="Balance to supply")
        self.qty_2 = self.create_label(text="Answer", color="#2CA02C")
        self.oversupplied_label = self.create_label(text="Oversupplied Before?")
        self.oversupplied = self.create_label(text="Answer", color="#2CA02C")
        
        # INPUT NEHR MEDICATION LIST DISPENSED    
        ...
        self.replace_widget(self.ui.load_pages.oversupplied_label, self.oversupplied_label)
        self.replace_widget(self.ui.load_pages.oversupplied, self.oversupplied)
    ...
    def handle_end_date_calc(self):
        ...
        duration = self.duration_2.text().strip()
    ...
    def handle_process(self):
        ...
        # 2. Get Tracker Parameters from UI Widgets
        try:
            start_date_str = self.start_date_2.date().toString("dd-MM-yyyy")
            duration = int(self.duration_2.text().strip())
            # We'll use qty_1 (tabs per day) as the dosage
            daily_dosage = float(self.qty_1.text().strip())
        
            # 3. Initialize the Engine with the parsed NEHR df
            # Pick the first medication found in the totals, or a specific selection
            if not totals.empty:
                ...
                tracker_df=engine.generate_report()

                # --- DISPLAYING OVERSUPPLIED LABEL ---
                # Check if "yes" exists anywhere in the 'oversupplied' column (case-insensitive)
                is_oversupplied = tracker_df['Oversupplied'].str.contains('yes', case=False).any()
                if is_oversupplied:
                    self.set_label_status(self.oversupplied, 'Yes', is_valid=False)
                else:
                    self.set_label_status(self.oversupplied, 'No', is_valid=True)

                # Calculate and display the supply balance
                needed_today = engine.calculate_supply_gap()
